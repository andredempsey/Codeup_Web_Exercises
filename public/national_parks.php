<?php
	...
	//checks if field is blank and updates error message; except Description field
	...
	//determine total pages for entire data set; not using ceiling at this point because need to preserve
	//the exact pages for calculating/displaying total number of records in the database below
	...
	if (isset($_GET['Page']))
	{
		if ($_GET['Page'] > ceil($totalPages))
		{
			...
			header("Location: national_parks.php?Page=$pageNumber");
			exit();
...
<body>
	...
	<div class = "container">
		...
		<form method="GET" action="/national_parks.php">
			...
			<ul class="pagination">
			  <li><a href="national_parks.php?Page=<?=$pageNumber-1?>">&laquo;</a></li>
			  	<?for ($i = 1; $i <= ceil($totalPages); $i++) : ?>
			  	<li><a href="national_parks.php?Page=<?=$i?>"><?=$i?></a></li>
				<?endfor;?>
			  <li><a href="national_parks.php?Page=<?=$pageNumber+1?>">&raquo;</a></li>
			</ul>
			</div>
		</form>
	...
	<div class = "container">
		<form method="POST" action="/national_parks.php">
			<p>
	            ...
	            <input  id="DateEst" name="DateEst" type="text" placeholder='YYYY-MM-DD' value=<?=(!empty($_POST) && $_POST['DateEst']!='')?$_POST['DateEst']:'' ?> >
